defmodule Cinder.TableV2 do
  ...
  ## Basic Usage
      ...
      <Cinder.TableV2.table resource={MyApp.User} current_user={@current_user}>
  ...
  def table(assigns) do
    ...
    ~H"""
    <div class={["cinder-table-v2", @class]}>
      <.live_component
  ...
  defp process_columns(col_slots, resource) do
    Enum.map(col_slots, fn slot ->
      # Convert new API slot to old API format using Column module
      ...
      # Create slot in old API format with proper label handling
      %{
        key: field,
  ...
  ## Examples
      ...
      <Cinder.TableV2.table resource={User} current_user={@user} class={responsive_classes()}>
