defmodule Cinder.Filters.MultiCheckboxes do
  ...
  def render(column, current_value, theme, _assigns) do
    ...
    ~H"""
    <div class={@theme.filter_multicheckboxes_container_class}>
      <div :for={{label, value} <- @options} class={@theme.filter_multicheckboxes_option_class}>
        <input
          type="checkbox"
        ...
        />
        <label class={@theme.filter_multicheckboxes_label_class}>{label}</label>
      </div>
    </div>
    """
  end
